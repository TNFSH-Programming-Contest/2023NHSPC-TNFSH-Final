...
\begin{frame}[fragile]{\btitle{子任務}}
	\begin{enumerate}
		...
		\item $N,Q \leq 5000$
		\item $\forall x_i,c_i \leq 10$
		\item $\forall x_i = 1$
		\item $N,Q \leq 300000$
	\end{enumerate}
\end{frame}

\begin{frame}[fragile]{\btitle{子任務 1 --- $N,Q \leq 5000$}}
	ㄜ...純粹暴力拿分
...
\begin{frame}[fragile]{\btitle{子任務 2 --- $\forall x_i,c_i \leq 10$}}
	\onslide<1->{
		發現值域全部都不會超過 $10$
	}
	
	\onslide<2->{
		可以開一個線段樹每個節點第 $i$ 個 bit 表示區間有沒有出現 $i$
	}

	\onslide<3->{
		query 區間直接把所有區間 or 起來算幾個 bit 是 $1$
...
\begin{frame}[fragile]{\btitle{子任務 3 --- $\forall x_i = 1$}}
	\onslide<1->{
		每次詢問一段區間的同時也會把那段區間全部改成一樣的
	}
	...
	\onslide<3->{
		每次 query 就直接暴力查詢還沒被改成 $1$ 的點

		修改就把它改成 $1$，暴力查詢的部分可以用 set, dsu, map...維護

		複雜度在均攤下是 $O(N\log N)\ or\ O(N \alpha (N))$
	}
\end{frame}
...
\begin{frame}[fragile]{\btitle{子任務 4 --- 無額外限制}}
	...
	\onslide<2->{
		每一次操作最多讓兩個相鄰不一樣的次數 $+2$

		總共的相鄰不一樣的次數最多 $N+2Q$
	}

	\onslide<3->{
		如果每個相鄰不同的情況都用線段樹 or set 去做修改和查詢的話
